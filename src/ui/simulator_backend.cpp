...
class SimulatorCanvas : public CanvasWrapper
{
  ...
  SimulatorCanvas() : w(0), h(0), buffer(nullptr), textFg(0xFFFF), textBg(0)
  {
    ...
    TTF_Init();
    window = SDL_CreateWindow("M5Stack Simulator", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
                              320, 240, 0);
    renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_SOFTWARE);
    texture =
        SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGB565, SDL_TEXTUREACCESS_STREAMING, 320, 240);
  ...
  void drawString(const char* str, int16_t x, int16_t y) override
  {
    ...
    SDL_Surface* surface = TTF_RenderText_Solid(
        font, str, {(textFg >> 11) * 8, ((textFg >> 5) & 63) * 4, (textFg & 31) * 8, 255});
  ...
  {
    if (!this->font) return;
    if (font == &m5::fonts::Font2)
      TTF_SetFontSize(this->font, 12);
    else if (font == &m5::fonts::Font4)
      TTF_SetFontSize(this->font, 24);
    else if (font == &m5::fonts::Font8)
