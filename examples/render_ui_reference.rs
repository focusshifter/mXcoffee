...
use mxcoffee::ui::{
    ...
    CLASSIC, HEIGHT, HISTORY_LEN, LCARS, NGE, WIDTH, WORKSHOP,
};
...
fn main() {
    ...
    let skin = if arguments.iter().any(|argument| argument == "--lcars") {
        &LCARS
    } else if arguments.iter().any(|argument| argument == "--nge") {
        &NGE
